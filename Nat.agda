...
--}}}
  ...
  a+b*c=a*c+b*c :
    (a b c : ℕ) -> 
    (a + b) * c ≡ (a * c) + (b * c)
  a+b*c=a*c+b*c zero zero zero =
    refl
  a+b*c=a*c+b*c zero (suc b) zero =
    refl
  a+b*c=a*c+b*c (suc a) zero zero =
    begin
      (a + zero) * zero
        ≡⟨ cong
             (λ x -> x * 0)
      ...
      a * 0
        ≡⟨ a=a+0 ⟩
      a * zero + 0 ∎
  a+b*c=a*c+b*c (suc a) (suc b) zero =
    begin
      (a + suc b) * zero
        ≡⟨ cong
             (λ x -> x * zero)
      ...
      (suc (a + b)) * zero
        ≡⟨ refl ⟩
      (a + b) * zero
        ≡⟨ a+b*c=a*c+b*c a b zero ⟩
      a * zero + b * zero ∎
  a+b*c=a*c+b*c zero zero (suc c) =
    refl
  a+b*c=a*c+b*c zero (suc b) (suc c) =
    refl
  a+b*c=a*c+b*c (suc a) zero (suc c) =
    begin
      (suc (c + (a + zero) * suc c))
        ≡⟨ cong
             (λ x -> (suc (c + x * suc c)))
      ...
      (suc (c + a * suc c))
        ≡⟨ cong
             ...
             a=a+0 ⟩
      (suc (c + a * suc c + zero)) ∎
  a+b*c=a*c+b*c (suc a) (suc b) (suc c) =
    cong
      suc
      (begin
        c + (a + suc b) * suc c
          ≡⟨ cong
               (λ x -> c + x)
               (a+b*c=a*c+b*c a (suc b) (suc c)) ⟩
        c + (a * suc c + (suc b * suc c))
          ≡⟨ sym (+-assoc c (a * suc c) (suc (c + b * suc c))) ⟩
        c + a * suc c + (suc b * suc c) ∎)

  c*a+b=c*a+c*b :
    (a b c : ℕ) ->
    c * (a + b) ≡ c * a + c * b
  c*a+b=c*a+c*b a b zero = refl
  c*a+b=c*a+c*b a b (suc c) =
    begin
      a + b + c * (a + b)
        ≡⟨ cong
             (λ x -> a + b + x)
